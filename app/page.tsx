...
const DiningHallButton = ({ name, score }: { name: string; score: number }) => {
  return (
    <button className="w-[850px] h-32 text-4xl font-bold text-white bg-[#599CDF] rounded-lg shadow-lg hover:bg-blue-800 flex items-center space-x-10">
      {/* Dynamic Score Box with Label Below */}
      <div className="w-20 h-20 bg-gray-300 text-black flex flex-col items-center justify-center ml-10 mr-6 rounded-md">
        <span className="text-3xl font-bold">{score}</span>
        <span className="text-xs font-semibold text-black ">Avg Daily Score</span>
      </div>
      <span>{name}</span>
    </button>
  );
