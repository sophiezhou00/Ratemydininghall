  ...
  useEffect(() => {
    const fetchResponses = async () => {
      try {
        ...
        // Filter responses where diningHall is "Carm"
        const carmResponses = data.filter((response: any) => response.diningHall === "Carmichael");
        setResponses(carmResponses);

        // Set overall rating (all-time, not date-dependent)
        updateOverallRating(carmResponses);

        // Set today's rating initially
        updateTodaysRating(carmResponses, selectedMeal);
      } catch (error) {
      ...
      <div className="flex w-full flex-col items-start mt-40">
        <div className="flex justify-between items-center w-full pb-12 px-16">
          <div className="flex items-center space-x-4">
            <p className="text-5xl font-spartan font-bold">Carmichael</p>
